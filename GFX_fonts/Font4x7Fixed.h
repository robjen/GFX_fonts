const uint8_t Bitmaps4x7Fixed[] PROGMEM = {
  
};

const GFXglyph Glyphs4x7Fixed[] PROGMEM = {
  {     0,   0,   1,   2,    0,    0 }   // ' '
 ,{     0,   1,   7,   2,    0,   -7 }   // '!'
 ,{     1,   3,   2,   4,    0,   -7 }   // '"'
 ,{     2,   4,   7,   5,    0,   -7 }   // '#'
 ,{     6,   3,   7,   4,    0,   -7 }   // '$'
 ,{    10,   3,   7,   4,    0,   -7 }   // '%'
 ,{    14,   4,   7,   5,    0,   -7 }   // '&'
 ,{    18,   2,   3,   3,    0,   -7 }   // '''
 ,{    20,   2,   7,   3,    0,   -7 }   // '('
 ,{    23,   2,   7,   3,    0,   -7 }   // ')'
 ,{    26,   3,   3,   4,    0,   -5 }   // '*'
 ,{    28,   3,   3,   4,    0,   -5 }   // '+'
 ,{    30,   2,   2,   3,    0,   -2 }   // ','
 ,{    31,   3,   1,   4,    0,   -4 }   // '-'
 ,{    32,   2,   2,   3,    0,   -2 }   // '.'
 ,{    33,   3,   7,   4,    0,   -7 }   // '/'
 ,{    37,   3,   7,   4,    0,   -7 }   // '0'
 ,{    41,   3,   7,   4,    0,   -7 }   // '1'
 ,{    45,   3,   7,   4,    0,   -7 }   // '2'
 ,{    49,   3,   7,   4,    0,   -7 }   // '3'
 ,{    53,   3,   7,   4,    0,   -7 }   // '4'
 ,{    57,   3,   7,   4,    0,   -7 }   // '5'
 ,{    61,   3,   7,   4,    0,   -7 }   // '6'
 ,{    65,   3,   7,   4,    0,   -7 }   // '7'
 ,{    69,   3,   7,   4,    0,   -7 }   // '8'
 ,{    73,   3,   7,   4,    0,   -7 }   // '9'
 ,{    77,   1,   5,   2,    0,   -6 }   // ':'
 ,{    79,   1,   6,   2,    0,   -6 }   // ';'
 ,{    81,   3,   5,   4,    0,   -6 }   // '<'
 ,{    84,   3,   3,   4,    0,   -5 }   // '='
 ,{    86,   3,   5,   4,    0,   -6 }   // '>'
 ,{    89,   3,   7,   4,    0,   -7 }   // '?'
 ,{    93,   4,   7,   5,    0,   -7 }   // '@'
 ,{    97,   3,   7,   4,    0,   -7 }   // 'A'
 ,{   101,   3,   7,   4,    0,   -7 }   // 'B'
 ,{   105,   3,   7,   4,    0,   -7 }   // 'C'
 ,{   109,   3,   7,   4,    0,   -7 }   // 'D'
 ,{   113,   3,   7,   4,    0,   -7 }   // 'E'
 ,{   117,   3,   7,   4,    0,   -7 }   // 'F'
 ,{   121,   4,   7,   5,    0,   -7 }   // 'G'
 ,{   125,   3,   7,   4,    0,   -7 }   // 'H'
 ,{   129,   3,   7,   4,    0,   -7 }   // 'I'
 ,{   133,   3,   7,   4,    0,   -7 }   // 'J'
 ,{   137,   3,   7,   4,    0,   -7 }   // 'K'
 ,{   141,   3,   7,   4,    0,   -7 }   // 'L'
 ,{   145,   4,   7,   5,    0,   -7 }   // 'M'
 ,{   149,   4,   7,   5,    0,   -7 }   // 'N'
 ,{   153,   3,   7,   6,    0,   -7 }   // 'O'
 ,{   157,   3,   7,   4,    0,   -7 }   // 'P'
 ,{   161,   3,   7,   4,    0,   -7 }   // 'Q'
 ,{   165,   3,   7,   4,    0,   -7 }   // 'R'
 ,{   169,   3,   7,   4,    0,   -7 }   // 'S'
 ,{   173,   3,   7,   4,    0,   -7 }   // 'T'
 ,{   177,   3,   7,   4,    0,   -7 }   // 'U'
 ,{   181,   3,   7,   4,    0,   -7 }   // 'V'
 ,{   185,   4,   7,   5,    0,   -7 }   // 'W'
 ,{   189,   3,   7,   4,    0,   -7 }   // 'X'
 ,{   193,   3,   7,   4,    0,   -7 }   // 'Y'
 ,{   197,   3,   7,   4,    0,   -7 }   // 'Z'
 ,{   201,   3,   7,   4,    0,   -7 }   // '['
 ,{   205,   3,   7,   4,    0,   -7 }   // '\'
 ,{   209,   3,   7,   4,    0,   -7 }   // ']'
 ,{   213,   3,   3,   4,    0,   -7 }   // '^'
 ,{   215,   3,   1,   4,    0,   -1 }   // '_'
 ,{   216,   2,   2,   3,    0,   -7 }   // '`'
 ,{   217,   3,   5,   4,    0,   -5 }   // 'a'
 ,{   220,   3,   7,   4,    0,   -7 }   // 'b'
 ,{   224,   3,   5,   4,    0,   -5 }   // 'c'
 ,{   227,   3,   7,   4,    0,   -7 }   // 'd'
 ,{   231,   3,   5,   4,    0,   -5 }   // 'e'
 ,{   234,   3,   7,   4,    0,   -7 }   // 'f'
 ,{   238,   3,   5,   4,    0,   -5 }   // 'g'
 ,{   241,   3,   7,   4,    0,   -7 }   // 'h'
 ,{   245,   1,   7,   2,    0,   -7 }   // 'i'
 ,{   246,   2,   7,   3,    0,   -7 }   // 'j'
 ,{   249,   3,   7,   4,    0,   -7 }   // 'k'
 ,{   253,   1,   7,   2,    0,   -7 }   // 'l'
 ,{   255,   4,   5,   5,    0,   -5 }   // 'm'
 ,{   258,   3,   5,   4,    0,   -5 }   // 'n'
 ,{   261,   3,   5,   4,    0,   -5 }   // 'o'
 ,{   264,   3,   5,   4,    0,   -5 }   // 'p'
 ,{   267,   3,   5,   4,    0,   -5 }   // 'q'
 ,{   270,   3,   5,   4,    0,   -5 }   // 'r'
 ,{   273,   3,   5,   4,    0,   -5 }   // 's'
 ,{   276,   3,   7,   4,    0,   -7 }   // 't'
 ,{   280,   3,   5,   4,    0,   -5 }   // 'u'
 ,{   283,   3,   5,   4,    0,   -5 }   // 'v'
 ,{   286,   4,   5,   5,    0,   -5 }   // 'w'
 ,{   289,   3,   5,   4,    0,   -5 }   // 'x'
 ,{   292,   3,   5,   4,    0,   -5 }   // 'y'
 ,{   295,   3,   5,   4,    0,   -5 }   // 'z'
 ,{   298,   3,   7,   4,    0,   -7 }   // '{'
 ,{   302,   1,   7,   2,    0,   -7 }   // '|'
 ,{   304,   3,   7,   4,    0,   -7 }   // '}'
};

const GFXfont Font4x7Fixed PROGMEM = {
(uint8_t  *)Bitmaps4x7Fixed,(GFXglyph *)Glyphs4x7Fixed,0x20, 0x7E, 7};
